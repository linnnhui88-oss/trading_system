...
def api_trades():
    """获取交易记录 (升级为展示实际成交 Trade Fills)"""
    try:
        repo = get_trade_fill_repo()
        limit = request.args.get('limit', 50, type=int)
        # list_fills 支持返回所有 action_type 的实际成交记录(包含部分平仓)
...
def api_market_dashboard():
    """获取仪表盘行情快照（包含当前价格、K线、买卖盘深度）"""
    try:
        symbol = request.args.get('symbol', 'BTCUSDT')
        interval = request.args.get('interval', '5m')
        # kline_limit 和 depth_limit 均已在 service 里有默认设定
...
ng_core.trade_fill_repository import TradeFillRepository
from trading_core.strategy_config_repository import StrategyConfigRepository
...
def close_position():
    ...
    try:
        ...
        # 改为通过 order_executor 平仓，以确保交易动作被写入 TradeFillRepository
        success = order_executor.close_position(symbol) if hasattr(order_executor, 'close_position') else exchange.close_position(symbol)
        return jsonify({
            'success': success,
            'message': f'{symbol} 平仓成功' if success else f'{symbol} 平仓失败'
        })
